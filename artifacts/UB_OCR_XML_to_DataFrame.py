...
def OCR_XML_to_DataFrame(XML_file_path, req_fields):
    ...
    tab_names = []
    table_field_names = []
    if ".TRANS" or ".KEY" or ".parse" in XML_file_path:
                ...
                if field.attrib["Typ"] == "SINGLE":
                    ...
                    if Sin_field_name in req_fields: 
                        ...
                        for k in range(len(history_s)):
                            ...
                            df.loc[row, "Page"] = cords.attrib["Pg"]
    ...
    #                     print(idx, tab_field.attrib["Nm"])
                        table_field_names.append(tab_field.attrib["Nm"])
            # If required to Extract Specific Table Line Fields, pass the single line fileds list here
